...
$canonicalUrl = siteUrl('books.php'.($topicSlug?'?topic='.urlencode($topicSlug):'').($search?($topicSlug?'&':'?').'q='.urlencode($search):''));
...
if($topic) $breadcrumbs[]=['name'=>$topic['name'],'url'=>siteUrl('books.php?topic='.urlencode($topic['slug']))];
...
<?php if($search || $topicSlug): ?><a href="<?= siteUrl('books.php') ?>" class="btn btn-outline-secondary"><i class="bi bi-x"></i></a><?php endif; ?>
</div>
</form>
...
<div class="mt-2"><a href="<?= siteUrl('topic.php?slug='.urlencode($topic['slug'])) ?>" class="btn btn-sm btn-outline-primary">صفحه موضوع <i class="bi bi-arrow-left ms-1"></i></a></div>
</div>
<?php endif; ?>
...
<?php if($search||$topicSlug): ?><a href="<?= siteUrl('books.php') ?>" class="btn btn-outline-primary mt-2">همه کتاب‌ها</a><?php endif; ?>
</div>
<?php else: ?>
<div class="row g-4">
<?php foreach($books as $b):
  $bookSlug = $b['slug'] ?: $b['id'];
  $bookUrl = $b['slug'] ? siteUrl('book.php?slug='.urlencode($b['slug'])) : siteUrl('book.php?id='.$b['id']);
  $cover = $b['cover_image'] ?? '';
?>
...
<?php $tpcs=getTopicsForBook((int)$b['id']); if($tpcs): ?><div class="mt-2 d-flex flex-wrap gap-1"><?php foreach(array_slice($tpcs,0,2) as $tp): ?><a href="<?= siteUrl('books.php?topic='.urlencode($tp['slug'])) ?>" class="badge rounded-pill" style="background:#f0ece3;color:#7a6a3a;font-size:.68rem;font-weight:600"><?= sanitize($tp['name']) ?></a><?php endforeach; ?></div><?php endif; ?>
...
<li class="page-item <?= $i===$page?'active':'' ?>"><a class="page-link rounded-pill px-3" href="<?= siteUrl('books.php'.($qs?'?'.$qs:'')) ?>"><?= $i ?></a></li>
<?php endfor; ?>
</ul>
</nav>
<?php endif; ?>
