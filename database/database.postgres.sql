...
-- Single identity table (see database/database.mysql.sql for the rationale).
...
-- setting_key: the column must not be named "key" because MySQL reserves it.
-- Both schemas use the same name so application SQL stays driver-neutral.
...
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='settings' AND column_name='key') THEN
...
CREATE TABLE IF NOT EXISTS login_limits (
 limit_key VARCHAR(64) PRIMARY KEY, attempts INTEGER NOT NULL DEFAULT 0, expires_at TIMESTAMPTZ NOT NULL
);
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema=current_schema() AND table_name='login_limits' AND column_name='key') THEN
    ALTER TABLE login_limits RENAME COLUMN key TO limit_key;
  END IF;
END $$;
...
-- Legacy public accounts are migrated into `users` (role='user') by
-- includes/identity.php; the old table is renamed to members_migrated_v1.
ALTER TABLE post_topics ADD COLUMN IF NOT EXISTS is_primary SMALLINT NOT NULL DEFAULT 0;
INSERT INTO topics (name, slug, description, intro, sort_order, is_featured) VALUES
