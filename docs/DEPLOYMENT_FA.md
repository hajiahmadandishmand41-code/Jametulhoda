...
- تعویض فایل در فرم‌های edit اکنون حذف قبلی را صف می‌کند؛ worker پیش از حذف تمام مراجع را بررسی می‌کند و در صورت شکست UPDATE یا استفاده مشترک، حذف را لغو می‌کند. مدیریت رسانه نیز فایل در حال استفاده را حذف نمی‌کند. **محدودیت باقی‌مانده:** rollback کامل uploadهای جدید و تغییر محتوای چندمرحله‌ای و reconciliation فایل‌های بدون مرجع باید تکمیل شود.
