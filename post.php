<?php
/**
 * post.php — صفحه مطلب (گزارش/مقاله/پژوهش/اطلاعیه) محتوامحور + SEO
 */
...
$slug = trim($_GET['slug'] ?? '');
if (!$slug) { header('Location: '.siteUrl('articles.php')); exit; }
...
if(($post['post_type'] ?? '')==='speech'){ redirect(siteUrl('speech.php?slug='.urlencode($post['slug']))); }
...
$typeMap=['news'=>['label'=>'اخبار','url'=>siteUrl('news.php')],'article'=>['label'=>'مقالات','url'=>siteUrl('articles.php')],'research'=>['label'=>'پژوهش‌ها','url'=>siteUrl('research.php')],'report'=>['label'=>'گزارش‌ها','url'=>siteUrl('reports.php')],'announcement'=>['label'=>'اطلاعیه‌ها','url'=>siteUrl('announcements.php')],'qa'=>['label'=>'پرسش و پاسخ','url'=>siteUrl('qa.php')]];
...
if($primaryTopic){
    foreach(getTopicBreadcrumbs((int)$primaryTopic['id']) as $bt){
        $breadcrumbs[]=['name'=>$bt['name'],'url'=>siteUrl('topic.php?slug='.urlencode($bt['slug']))];
    }
}
...
<div class="d-flex flex-wrap gap-2 mb-2"><?= postTypeBadge($post['post_type']) ?> <?php if($post['cat_name']): ?><a href="<?= siteUrl('category.php?slug='.urlencode($post['cat_slug'])) ?>" class="badge bg-secondary"><?= sanitize($post['cat_name']) ?></a><?php endif; ?><?php if($primaryTopic): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($primaryTopic['slug'])) ?>" class="badge" style="background:#fdf6e3;color:#7a5a1a;border:1px solid #e8d5a3"><i class="bi bi-tag ms-1"></i><?= sanitize($primaryTopic['name']) ?></a><?php endif; ?></div>
...
<?php foreach($postTopics as $t): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($t['slug'])) ?>" class="badge bg-light text-dark border" style="font-size:.78rem"><i class="bi bi-folder ms-1"></i><?= sanitize($t['name']) ?></a><?php endforeach; ?>
</div>
<?php endif; ?>
</header>
...
<?php $firstVideo=$postVideos[0]; $videoPoster=$post['featured_image'] ? imgUrl($post['featured_image']) : ''; $firstVideoUrl=siteUrl($firstVideo['file_path']); ?>
...
<div class="playlist mt-3" id="postVideoPlaylist"><?php foreach($postVideos as $vi=>$v): ?><button type="button" class="playlist-item <?= $vi===0?'active':'' ?>" data-src="<?= htmlspecialchars(siteUrl($v['file_path']),ENT_QUOTES) ?>" data-index="<?= $vi ?>"><span class="pl-num"><?= $vi+1 ?></span><span class="pl-title"><?= sanitize($v['title'] ?: ('ویدیو '.($vi+1))) ?></span><i class="bi bi-play-circle-fill pl-icon"></i></button><?php endforeach; ?></div>
...
<audio id="mainAudio" controls preload="none" class="w-100"><source data-src="<?= siteUrl($postAudios[0]['file_path']) ?>" src="" type="audio/mpeg"></audio>
<div class="d-flex gap-2 mt-2 flex-wrap"><button type="button" class="btn btn-outline-primary btn-sm" id="audioPrevBtn"><i class="bi bi-skip-end-fill"></i> قبلی</button><button type="button" class="btn btn-outline-primary btn-sm" id="audioNextBtn">بعدی <i class="bi bi-skip-start-fill"></i></button><a href="<?= siteUrl($postAudios[0]['file_path']) ?>" download id="audioDownload" class="btn btn-outline-success btn-sm"><i class="bi bi-download ms-1"></i>دانلود</a></div>
<?php if(count($postAudios)>1): ?><div class="playlist mt-3" id="audioPlaylist"><?php foreach($postAudios as $ai=>$a): ?><button type="button" class="playlist-item <?= $ai===0?'active':'' ?>" data-src="<?= siteUrl($a['file_path']) ?>" data-index="<?= $ai ?>"><span class="pl-num"><?= $ai+1 ?></span><span class="pl-title"><?= sanitize($a['title'] ?: ('صوت '.($ai+1))) ?></span><i class="bi bi-play-circle-fill pl-icon"></i></button><?php endforeach; ?></div><?php endif; ?>
</div>
<?php endif; ?>
...
<!-- Internal linking: topics → other contents -->
<?php if($postTopics): ?>
