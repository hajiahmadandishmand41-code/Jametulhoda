...
<div class="col-md-6"><div class="card h-100 p-3"><span class="badge bg-primary mb-2">پرسش و پاسخ</span><h2 class="h6"><a href="<?= siteUrl('post.php?slug='.urlencode($p['slug'])) ?>"><?= sanitize($p['title']) ?></a></h2><p class="text-muted small"><?= sanitize(excerpt($p['summary'] ?? '',120)) ?></p><a href="<?= siteUrl('post.php?slug='.urlencode($p['slug'])) ?>" class="btn-read-more">مشاهده پاسخ <i class="bi bi-arrow-left"></i></a></div></div>
<?php endforeach; ?></div>
<?php if($pages>1): ?><div class="mt-4"><?= paginate($total,$limit,$page, siteUrl('qa.php?q='.urlencode($search).'&page=%d')) ?></div><?php endif; ?>
<?php endif; ?>
</div></div>
