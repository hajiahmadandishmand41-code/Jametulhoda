...
// Rows sourced from media_files carry their id so cards link to the media
// detail page (/video/{id}, /audio/{id}); embedded lesson/post files link to
// their parent detail page instead.
...
if($kind==='video') $sql.=" UNION SELECT NULL,p.featured_video,p.title,p.slug,p.post_type,p.featured_image,p.published_at,'post' FROM posts p WHERE p.status='published' AND p.featured_video IS NOT NULL AND p.featured_video<>''";
...
<?php foreach($items as $item):
  if(!empty($item['media_id'])) $detailUrl=mediaUrl($kind,(int)$item['media_id']);
  elseif($item['target']==='lesson') $detailUrl=lessonUrl($item['slug']);
  else $detailUrl=postUrl(['slug'=>$item['slug'],'post_type'=>$item['post_type'] ?? 'post']);
?><div class="col-md-6 col-lg-4"><article class="card h-100">
...
<div class="card-body"><small class="text-muted"><?= sanitize(persianDate($item['date'])) ?></small><h2 class="h5 mt-2"><a href="<?= $detailUrl ?>"><?= sanitize($item['title']) ?></a></h2>
<?php if($kind==='audio'): ?><audio controls preload="none" aria-label="<?= sanitize($item['title']) ?>" src="<?= imgUrl($item['path']) ?>"></audio><?php endif; ?>
<a class="jhd-text-link mt-3" href="<?= $detailUrl ?>">جزئیات و متن جلسه <i class="bi bi-arrow-left"></i></a></div></article></div><?php endforeach; ?>
</div>
