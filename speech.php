...
if (!$post || $post['post_type'] !== 'speech') {
    ...
    echo '<div class="container py-5 text-center"><h2>سخنرانی مورد نظر یافت نشد</h2><a href="' . siteUrl('speeches.php') . '" class="btn btn-primary mt-3">بازگشت به سخنرانی‌ها</a></div>';
        ...
        <div class="col-lg-8">
            <article class="single-post">
                ...
                <div class="post-nav mt-5 d-flex justify-content-between gap-2 flex-wrap">
                    <?php if ($prevRow): ?>
                    <a href="<?= siteUrl('speech.php?slug=' . urlencode($prevRow['slug'])) ?>" class="btn btn-outline-secondary">
                        <i class="bi bi-arrow-right ms-1"></i> سخنرانی قبلی: <?= sanitize(mb_strimwidth($prevRow['title'],0,30,'...')) ?>
                    </a>
                    <?php else: ?><span></span><?php endif; ?>
                    <?php if ($nextRow): ?>
                    <a href="<?= siteUrl('speech.php?slug=' . urlencode($nextRow['slug'])) ?>" class="btn btn-outline-secondary">
            ...
            <div class="sidebar">
                <div class="sidebar-widget">
                    ...
                    <ul class="sidebar-list">
                        ...
                        <li>
                            <a href="<?= siteUrl('speech.php?slug=' . urlencode($os['slug'])) ?>"><?= sanitize(mb_strimwidth($os['title'],0,55,'...')) ?></a>
