...
<div class="article-card-footer"><a href="<?= postUrl($a) ?>" class="btn-read-more">مطالعه مقاله <i class="bi bi-arrow-left"></i></a></div>
</div>
</article>
</div>
<?php endforeach; ?>
...
<a href="<?= topicUrl($featuredTopic) ?>" class="btn btn-primary">مشاهده همه مطالب موضوع <i class="bi bi-arrow-left ms-1"></i></a>
</div>
<?php $children=getTopicChildren((int)$featuredTopic['id']); if($children): ?>
<div class="d-flex flex-wrap gap-2 mb-4"><?php foreach($children as $ch): ?><a href="<?= topicUrl($ch) ?>" class="badge bg-light text-dark border" style="padding:8px 12px;font-size:.82rem"><?= sanitize($ch['name']) ?></a><?php endforeach; ?></div>
<?php endif; ?>
<div class="row g-4">
<div class="col-lg-4">
...
<div class="list-post-item mb-2"><div style="width:56px;height:70px;flex-shrink:0;border-radius:6px;overflow:hidden;background:#fdf6e3;display:flex;align-items:center;justify-content:center"><?php if($b['cover_image']): ?><img src="<?= imgUrl($b['cover_image']) ?>" style="width:100%;height:100%;object-fit:cover" loading="lazy"><?php else: ?><i class="bi bi-book text-warning"></i><?php endif; ?></div><div class="list-post-info"><a href="<?= bookUrl($b) ?>" class="list-post-title"><?= sanitize($b['title']) ?></a><?php if($b['author']): ?><span class="list-post-date"><?= sanitize($b['author']) ?></span><?php endif; ?></div></div>
<?php endforeach; endif; ?>
</div>
<div class="col-lg-4">
...
<div class="list-post-item mb-2"><div style="width:56px;height:56px;flex-shrink:0;border-radius:8px;background:#eef2ff;display:flex;align-items:center;justify-content:center"><i class="bi bi-mortarboard text-primary"></i></div><div class="list-post-info"><a href="<?= lessonUrl($ls) ?>" class="list-post-title"><?= sanitize($ls['title']) ?></a><span class="list-post-date"><?php if($ls['teacher']): ?>استاد: <?= sanitize($ls['teacher']) ?> <?php endif; ?></span></div></div>
<?php endforeach; endif; ?>
</div>
</div>
</div>
...
<h6 class="announcement-title mb-1"><a href="<?= postUrl($ann) ?>"><?= sanitize($ann['title']) ?></a></h6>
