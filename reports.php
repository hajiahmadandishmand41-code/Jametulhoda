...
$breadcrumbs=[['name'=>'صفحه اصلی','url'=>siteUrl()],['name'=>'گزارش‌ها','url'=>siteUrl('reports.php')]];
if($topicId && $t) $breadcrumbs[]=['name'=>$t['name'],'url'=>siteUrl('reports.php?topic='.urlencode($t['slug']))];
...
<h2 class="news-card-title h5"><a href="<?= siteUrl('post.php?slug='.urlencode($p['slug'])) ?>"><?= sanitize($p['title']) ?></a></h2>
<?php if($p['summary']): ?><p class="news-card-summary"><?= sanitize(excerpt($p['summary'],110)) ?></p><?php endif; ?>
<?php $pt=getTopicsForPost((int)$p['id']); if($pt): ?><div class="d-flex flex-wrap gap-1 mt-2"><?php foreach(array_slice($pt,0,2) as $tp): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($tp['slug'])) ?>" class="badge bg-light text-dark border" style="font-size:.70rem"><?= sanitize($tp['name']) ?></a><?php endforeach; ?></div><?php endif; ?>
<div class="news-card-footer"><a href="<?= siteUrl('post.php?slug='.urlencode($p['slug'])) ?>" class="btn-read-more">مشاهده گزارش <i class="bi bi-arrow-left ms-1"></i></a></div>
</div>
</article>
</div>
<?php endforeach; ?>
</div>
<?php if($pages>1): ?><div class="mt-5"><?= paginate($total,$limit,$page, siteUrl('reports.php?q='.urlencode($search).'&topic='.urlencode($topicSlug).'&page=%d')) ?></div><?php endif; ?>
<?php endif; ?>
</div></div>
