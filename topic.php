...
$slug=trim($_GET['slug'] ?? '');
if(!$slug){ header('Location: '.siteUrl('topics.php')); exit; }
$topic=getTopicBySlug($slug);
if(!$topic || !$topic['is_active']){
    http_response_code(404);
    $pageTitle='موضوع یافت نشد';
    require_once __DIR__.'/includes/header.php';
    echo '<div class="container py-5 text-center"><h1>موضوع یافت نشد</h1><a href="'.siteUrl('topics.php').'" class="btn btn-primary mt-3">همه موضوعات</a></div>';
...
$breadcrumbs=[['name'=>'صفحه اصلی','url'=>SITE_URL? rtrim(SITE_URL,'/').'/': siteUrl()],['name'=>'موضوعات','url'=>siteUrl('topics.php')]];
foreach($parents as $p){
    if($p['id']==$topic['id']) continue;
    $breadcrumbs[]=['name'=>$p['name'],'url'=>siteUrl('topic.php?slug='.urlencode($p['slug']))];
}
...
<a href="<?= siteUrl('topic.php?slug='.urlencode($ch['slug'])) ?>" class="btn btn-sm" style="background:var(--jhd-surface);border:1px solid var(--jhd-border)"><?= sanitize($ch['name']) ?> <i class="bi bi-arrow-left ms-1"></i></a>
<?php endforeach; ?>
</div>
<?php endif; ?>
</div>
...
<section class="mb-5"><div class="d-flex justify-content-between align-items-center mb-3"><h2 class="h5 fw-bold"><i class="bi bi-newspaper ms-2 text-success"></i> گزارش‌ها</h2><a href="<?= siteUrl('reports.php?topic='.urlencode($topic['slug'])) ?>" class="btn btn-sm btn-outline-secondary">همه گزارش‌های این موضوع</a></div>
...
<div class="news-card-body"><h3 class="news-card-title"><a href="<?= siteUrl('post.php?slug='.urlencode($r['slug'])) ?>"><?= sanitize($r['title']) ?></a></h3><p class="news-card-summary"><?= sanitize(excerpt($r['summary'] ?? '',90)) ?></p><a href="<?= siteUrl('post.php?slug='.urlencode($r['slug'])) ?>" class="btn-read-more">ادامه مطلب <i class="bi bi-arrow-left"></i></a></div>
</article></div>
...
if($totalCount===0): ?>
<div class="text-center py-5 border rounded" style="background:var(--jhd-paper)"><i class="bi bi-inbox display-4 d-block mb-2 opacity-25"></i><p class="text-muted">هنوز محتوایی برای این موضوع ثبت نشده است.</p><a href="<?= siteUrl('topics.php') ?>" class="btn btn-outline-secondary btn-sm mt-2">مشاهده سایر موضوعات</a></div>
<?php endif; ?>
