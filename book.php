...
if(!$book || ($book['status']??'published')!=='published'){
    ...
    echo '<section class="container py-5"><nav class="breadcrumb-bar mb-4"><a href="'.siteUrl('books.php').'">کتابخانه</a> / یافت نشد</nav><h1>کتاب مورد نظر یافت نشد.</h1><a class="btn btn-primary mt-3" href="'.siteUrl('books.php').'">کتابخانه</a></section>';
...
$canonicalUrl = $book['slug'] ? siteUrl('book.php?slug='.urlencode($book['slug'])) : siteUrl('book.php?id='.$book['id']);
$ogImage   = !empty($book['cover_image']) ? imgUrl($book['cover_image']) : null;
$ogType    = 'book';
$breadcrumbs = [
  ['name'=>'صفحه اصلی','url'=>siteUrl()],
  ['name'=>'کتابخانه','url'=>siteUrl('books.php')],
...
<?php if(!empty($book['pdf_file'])): ?><a href="<?= siteUrl('book.php?'.($book['slug']?'slug='.urlencode($book['slug']):'id='.$book['id']).'&download=pdf') ?>" class="btn btn-primary"><i class="bi bi-file-pdf ms-2"></i> دانلود PDF</a><?php endif; ?>
<?php if(!empty($book['word_file'])): ?><a href="<?= siteUrl('book.php?'.($book['slug']?'slug='.urlencode($book['slug']):'id='.$book['id']).'&download=word') ?>" class="btn btn-outline-primary"><i class="bi bi-file-word ms-2"></i> دانلود Word</a><?php endif; ?>
</div>
<?php if($topics): ?>
<div class="text-start mt-4">
<div class="small fw-bold mb-2" style="color:var(--jhd-primary)"><i class="bi bi-tags ms-1"></i> موضوعات</div>
<div class="d-flex flex-wrap gap-2">
<?php foreach($topics as $tp): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($tp['slug'])) ?>" class="badge rounded-pill" style="background:#f0ece3;color:#5b4a1a;border:1px solid #e8e6dc"><?= sanitize($tp['name']) ?></a><?php endforeach; ?>
</div>
</div>
<?php endif; ?>
</div>
...
<div class="d-flex flex-wrap gap-2">
<?php foreach($topics as $tp): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($tp['slug'])) ?>" class="btn btn-sm btn-outline-secondary rounded-pill"><?= sanitize($tp['name']) ?></a><?php endforeach; ?>
</div>
</section>
<?php endif; ?>

<div class="d-flex flex-wrap gap-2 mt-4">
<a href="<?= siteUrl('books.php') ?>" class="btn btn-outline-secondary"><i class="bi bi-arrow-right ms-1"></i> بازگشت به کتابخانه</a>
<?php if($topics): $firstTopic=$topics[0]; ?><a href="<?= siteUrl('topic.php?slug='.urlencode($firstTopic['slug'])) ?>" class="btn btn-outline-primary">مشاهده در موضوع <?= sanitize($firstTopic['name']) ?></a><?php endif; ?>
</div>
</div>
</div>
...
<?php foreach($related as $rb):
  $rbUrl=$rb['slug']?siteUrl('book.php?slug='.urlencode($rb['slug'])):siteUrl('book.php?id='.$rb['id']);
?>
