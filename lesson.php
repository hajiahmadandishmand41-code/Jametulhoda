...
$slug = trim($_GET['slug'] ?? '');
if (!$slug) redirect(siteUrl('lessons.php'));
...
if(!$lesson){
    ...
    echo '<div class="container py-5 text-center"><h1>درس یافت نشد</h1><a href="'.siteUrl('lessons.php').'" class="btn btn-primary mt-3">بازگشت</a></div>';
...
if($lesson['collection_title']){
    $breadcrumbs[]=['name'=>$lesson['collection_title'],'url'=>siteUrl('lessons.php?collection='.urlencode($lesson['collection_slug']))];
    if($lesson['volume_title']) $breadcrumbs[]=['name'=>$lesson['volume_title'],'url'=>siteUrl('lessons.php?collection='.urlencode($lesson['collection_slug']).'&volume='.urlencode($lesson['volume_slug']))];
} elseif($lesson['subject']){
    $breadcrumbs[]=['name'=>sanitize($lesson['subject']),'url'=>siteUrl('lessons.php?q='.urlencode($lesson['subject']))];
}
...
<?php if($lesson['collection_title']): ?><a href="<?= siteUrl('lessons.php?collection='.urlencode($lesson['collection_slug'])) ?>" class="badge bg-primary"><?= sanitize($lesson['collection_title']) ?></a><?php endif; ?>
...
<?php if($lessonTopics): ?>
<div class="d-flex flex-wrap gap-1 mt-3"><?php foreach($lessonTopics as $t): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($t['slug'])) ?>" class="badge bg-light text-dark border"><i class="bi bi-tag ms-1"></i><?= sanitize($t['name']) ?></a><?php endforeach; ?></div>
<?php endif; ?>
</header>
...
<?php if($adjacent['prev']): ?><a href="<?= siteUrl('lesson.php?slug='.urlencode($adjacent['prev']['slug'])) ?>" class="btn btn-outline-secondary"><i class="bi bi-arrow-right ms-1"></i> درس قبلی: <?= sanitize(mb_strimwidth($adjacent['prev']['title'],0,25,'...')) ?></a><?php else: ?><span></span><?php endif; ?>
<?php if($adjacent['next']): ?><a href="<?= siteUrl('lesson.php?slug='.urlencode($adjacent['next']['slug'])) ?>" class="btn btn-primary">درس بعدی: <?= sanitize(mb_strimwidth($adjacent['next']['title'],0,25,'...')) ?> <i class="bi bi-arrow-left ms-1"></i></a><?php endif; ?>
</div>
<?php endif; ?>
...
<div class="d-flex flex-wrap gap-2"><?php foreach($lessonTopics as $t): ?><a href="<?= siteUrl('topic.php?slug='.urlencode($t['slug'])) ?>" class="badge bg-white text-dark border"><i class="bi bi-tag ms-1"></i><?= sanitize($t['name']) ?></a><?php endforeach; ?></div>
</div>
<?php endif; ?>
...
<li <?= $isCurrent?'style="background:#eaf3ec;border-radius:6px;padding:4px 6px"':'' ?>><a href="<?= siteUrl('lesson.php?slug='.urlencode($vl['slug'])) ?>" style="<?= $isCurrent?'color:#0d5a2b;font-weight:700':'' ?>"><?= sanitize($vl['title']) ?> <?php if($vl['lesson_number']): ?><small class="text-muted">— درس <?= (int)$vl['lesson_number'] ?></small><?php endif; ?></a></li>
<?php endforeach; ?>
</ul>
<?php endforeach; ?>
</div>
...
<div class="sidebar-widget"><h2 class="sidebar-title"><i class="bi bi-collection-play ms-2"></i> درس‌های مرتبط</h2><ul class="sidebar-list"><?php foreach($relatedLessons as $rl): ?><li><a href="<?= siteUrl('lesson.php?slug='.urlencode($rl['slug'])) ?>"><?= sanitize(mb_strimwidth($rl['title'],0,55,'...')) ?></a></li><?php endforeach; ?></ul></div>
<?php endif; ?>

<div class="sidebar-widget text-center"><a href="<?= siteUrl('lessons.php') ?>" class="btn btn-outline-primary w-100"><i class="bi bi-collection ms-1"></i> همه دروس</a></div>
</div>
</div>
</div></div></div>
